...
#if defined(NDEBUG)
# error "Outacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5f9e3ef9787d641959ede69cd7ff483c3d61852f25ad0335bd283a33893a19c9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Outacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Outacoin Signed Message:\n";
...
{
    // Outacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Outacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 14 * 24 * 60 / 10; // Outacoin: 2 weeks
static const int64 nTargetSpacing = 10 * 60; // Outacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Outacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Outacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7145388195e5f12befbce0e045dc02d464c7ccb60b7cc81ffbcca455f2a7d091");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This Is OutaCOin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0426bc9f0c34b5f5038f9dd7b50f818cafcea3fdf6aa9dd5098ad31319f9e6cd16722d74b6eccb5af5dfb40a9e3edb9b75b6266fb9eb1d64ae5c5528df18eb5a93") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514935121;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085340589;

        if (fTestNet)
        {
            block.nTime    = 1514935121;
            block.nNonce   = 385678350;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xbdfa46cd6099b3b468bfe54350bd53c22810448d5d2c18d6e21a35d19c0e10c9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc5, 0xbd, 0xd8 }; // Outacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OutacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OutacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OutacoinMiner(CWallet *pwallet)
{
    printf("OutacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("outacoin-miner");
    ...
    try { loop {
        ...
        printf("Running OutacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OutacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OutacoinMiner, pwallet));
}
